...
class _Conv_Block_0(nn.Module):    
    def __init__(self):
        super(_Conv_Block_0, self).__init__()
...
class DnCNN(nn.Module):
    def __init__(self, channels, num_of_layers=40):
        super(DnCNN, self).__init__()
        self.convt_I0 = self.make_layer(_Conv_Block_0)
        self.convt_R0 = nn.Conv2d(in_channels=64, out_channels=1, kernel_size=3, stride=1, padding=1, bias=False)

        self.conv_input = nn.Conv2d(in_channels=1, out_channels=64, kernel_size=3, stride=1, padding=1, bias=False)
        self.relu = nn.LeakyReLU(0.2, inplace=True)
        
        # in_channels 1->64
        self.convt_I1 = nn.ConvTranspose2d(in_channels=64, out_channels=1, kernel_size=4, stride=2, padding=1, bias=False)
    ...
    def forward(self, x):    
        mid = self.relu(self.conv_input(x))
        mid2 = self.convt_I0(mid)
        post = self.convt_R0(mid2)
        out = mid - mid2


        convt_F1 = self.convt_F1(out)
        convt_I1 = self.convt_I1(out)
        convt_R1 = self.convt_R1(convt_F1)
        HR_2x = convt_R1 + convt_I1
